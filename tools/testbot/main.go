...
func main() {
	...
	if err != nil {
		log.Warn().Err(err).Msg("failed to resolve mint")
		// Continue with empty mint or exit? Original code implied it always worked or crashed.
		// Let's exit to be safe as testbot relies on mint.
		log.Fatal().Err(err).Msg("failed to resolve mint")
	}
