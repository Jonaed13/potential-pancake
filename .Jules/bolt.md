...
## 2025-02-24 - Batching RPC calls in hot loop
**Learning:** Even with parallelization, making N RPC calls for N positions to check balances saturates the rate limit and adds unnecessary latency.
**Action:** Implemented `GetAllTokenAccounts` to fetch all balances in a single RPC call. This reduces the network overhead from O(N) to O(1) for balance checks in `monitorPositions`. Used `strconv` instead of `fmt` for parsing.
