...
func (cm ConfigModal) GetDescription(index int) string {
	switch index {
	case 0:
		return "Minimum signal strength (0-100) to enter a trade."
	case 1:
		return "Multiplier to exit trade (e.g. 1.5x = 50% profit)."
	case 2:
		return "Max percentage of portfolio to allocate per trade."
	case 3:
		return "Maximum number of concurrent open positions."
	case 4:
		return "SOL fee added to transaction to speed up processing."
	case 5:
		return "Master switch: ON=Bot trades, OFF=Signals only."
	default:
		return ""
	}
}
