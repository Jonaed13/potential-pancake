...
type Model struct {
	...
	PreviousScreen Screen // For context restoration
	Width, Height  int
...
func (m Model) renderHelp() string {
	s := StyleTableHeader.Render("HELP & KEYBINDINGS") + "\n\n"

	s += StyleTableHeader.Render("GLOBAL") + "\n"
	s += "?   Toggle Help    Q   Quit App\n"
	s += "P   Pause/Resume\n\n"

	s += StyleTableHeader.Render("NAVIGATION") + "\n"
	s += "Tab    Cycle Focus\n"
	s += "Arrows Scroll/Nav\n"
	s += "Esc    Back/Cancel\n\n"

	s += StyleTableHeader.Render("ACTIONS") + "\n"
	s += "C   Config       S   Sell All\n"
	s += "E   Export       F9  Reset All\n\n"

	s += StyleTableHeader.Render("VIEWS") + "\n"
	s += "1   Health       L   Logs\n"
	s += "2   Export       T   Trades\n"
	s += "3   Theme        4   Clear\n"
