...
func (e *ExecutorFast) monitorPositions(ctx context.Context) {
	...
	// Instead of N RPC calls (one per position), we do 1 call to get all balances.
	var balanceMap map[string]uint64
	// Only fetch real balances if not in simulation mode
	if !e.simMode && !e.cfg.Get().Trading.SimulationMode {
		accounts, err := e.rpc.GetAllTokenAccounts(ctx, e.wallet.Address())
		if err == nil {
			balanceMap = make(map[string]uint64, len(accounts))
			for _, acc := range accounts {
				balanceMap[acc.Mint] = acc.Amount
			}
		} else {
			log.Warn().Err(err).Msg("failed to batch fetch token accounts, falling back to individual checks")
		}
	}
	...
	for _, pos := range positions {
		...
		go func(pos *Position) {
			...
			// ⚡ Bolt Optimization: Use cached balance if available
			if val, ok := balanceMap[pos.Mint]; ok {
				balance = val
			} else {
				// Fallback to individual RPC (or simulation logic)
