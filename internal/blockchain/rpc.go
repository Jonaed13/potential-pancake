...
func (c *RPCClient) GetTokenAccountBalance(ctx context.Context, tokenAccount string) (uint64, uint8, error) {
	...
	if err != nil {
		return 0, 0, fmt.Errorf("parse amount: %w", err)
	}
...
func (c *RPCClient) GetTokenAccountsByOwner(ctx context.Context, owner, mint string) ([]TokenAccountInfo, error) {
	...
	for _, v := range result.Value {
		// ⚡ Bolt Optimization: Use strconv instead of fmt.Sscanf (30x faster)
		amount, err := strconv.ParseUint(v.Account.Data.Parsed.Info.TokenAmount.Amount, 10, 64)
		if err != nil {
			log.Warn().Err(err).Str("amount", v.Account.Data.Parsed.Info.TokenAmount.Amount).Msg("failed to parse token amount, skipping")
			continue
		}
